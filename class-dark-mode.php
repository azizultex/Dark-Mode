...
class Dark_Mode
{
    const darkModeCookieName = 'wordpress_dark_mode';
    ...
    {
        add_action('plugins_loaded', [__CLASS__, 'load_text_domain'], 10, 0);
        add_action('admin_enqueue_scripts', [__CLASS__, 'load_dark_mode_scripts']);
        add_action('admin_enqueue_scripts', [__CLASS__, 'load_dark_mode_css'], 99, 0);
        add_action('personal_options', [__CLASS__, 'add_profile_fields'], 10, 1);
        add_action('personal_options_update', [__CLASS__, 'save_profile_fields'], 10, 1);
        add_action('edit_user_profile_update', [__CLASS__, 'save_profile_fields'], 10, 1);

        add_filter('plugin_action_links', [__CLASS__, 'add_plugin_links'], 10, 2);
        add_filter('admin_body_class', [__CLASS__, 'add_body_class'], 10, 1);

        add_filter('is_using_dark_mode', [__CLASS__, 'dark_mode_auto'], 10, 2);
    }
    ...
    /**
     * Check automated dark mode setting
     *
     * @param bool $enabled Dark mode setting enabled
     * @param int  $user_id The user id
     *
     * @return void
     */
    public static function dark_mode_auto($enabled = false, $user_id = 0)
    {
        if (0 === $user_id) {
            $user_id = get_current_user_id();
        }

        // Need the cookie to fund this
        if (!isset($_COOKIE[self::darkModeCookieName])) {
            return $enabled;
        }

        // Check if the user is using Dark Mode.
        $system_dark_mode_enabled = filter_var($_COOKIE[self::darkModeCookieName], FILTER_VALIDATE_BOOLEAN);
        if ('on' === get_user_meta($user_id, 'dark_mode_auto', true)) {
            if ($enabled && !$system_dark_mode_enabled) {
                return false;
            }
        }
    }
    ...
    public static function load_dark_mode_css()
    {
        ...
        if (false !== self::is_using_dark_mode()) {
            ...
            wp_enqueue_style('dark_mode', $css_url, [], $ver);
        }
    }
    ...
    public static function add_profile_fields($user)
    {
        ...
        include plugin_dir_path(__FILE__) . 'partials/profile-fields.php';
    }
    ...
    public static function save_profile_fields($user_id)
    {
        if (isset($_POST['dark_mode_nonce']) && wp_verify_nonce(sanitize_key($_POST['dark_mode_nonce']), 'dark_mode_nonce')) {
            ...
            $option_auto = isset($_POST['dark_mode_auto']) ? 'on' : 'off';
             ...
             * @param string $option_auto The user's auto option choice.
             * @param int    $user_id     The current user's id.
             */
            $option = apply_filters('before_dark_mode_saved', $option, $option_auto, $user_id);

            // Update the users meta.
            update_user_meta($user_id, 'dark_mode', $option);
            update_user_meta($user_id, 'dark_mode_auto', $option_auto);
             ...
             * @param int    $user_id     The current user's id.
             */
            do_action('after_dark_mode_saved', $option, $option_auto, $user_id);
        }
    }
    ...
    /**
     * Load the admin dark mode script
     *
     * @return void
     */
    public static function load_dark_mode_scripts()
    {
        $ver = get_plugin_data(dirname(__FILE__) . '/dark-mode.php')['Version'];
        wp_enqueue_script('wp-dark-mode', plugin_dir_url(__FILE__) . 'scripts/wp-dark-mode.min.js', [], $ver);
    }
}
